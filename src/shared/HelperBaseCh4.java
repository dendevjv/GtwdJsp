...
public abstract class HelperBaseCh4 {
	...
	public HelperBaseCh4(HttpServlet servlet, HttpServletRequest request,
	...
	public void populateThrow(Object bean) throws IOException {
		try {
			org.apache.commons.beanutils.BeanUtils.populate(bean,
					request.getParameterMap());
		} catch (IllegalAccessException e) {
			logger.error("Populate - Illegal Access. ", e);
			writeError(request, response, "Populate - Illegal Access. ", e);
		} catch (InvocationTargetException e) {
			logger.error("Populate - Invocation Target. ", e);
			writeError(request, response, "Populate - Invocation Target. ", e);
		}
	}
